...
def create_tasks_database():
    ...
    # format is (url, compelted), where completed = 0 to indicate that the links
...
# just have that list of URLs be passed to this file
    urls = [
    "https://www.amazon.ca/",
...
# when trying to scrape "https://torontosun.com/news/local-news/warmington-bury-mcarthur-in-at-least-200-years-of-prison-time", 
# the program just hung with no error message. Look into why that happened
# when just using "request.get(url)" separately, it just loaded it like normal
